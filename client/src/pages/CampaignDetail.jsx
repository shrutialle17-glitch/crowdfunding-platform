...
export const CampaignDetail = () => {
  ...
  if (isLoading) 
    ...
    <div className="max-w-[1600px] mx-auto py-12 px-6">
      <div className="animate-pulse h-96 bg-border/50 rounded-3xl">
      </div>
    </div>;
  if (!campaign) 
    return 
    <div className="text-center py-20 text-h3">Campaign not found</div>;
  ...
  const percentFunded = campaign
    ? Math.min(
        Math.round(
          ((campaign.amountRaised || 0) / (campaign.fundingGoal || 1)) * 100,
        ),
        100,
      )
    : 0;
  ...
  return (
    <div className="max-w-[1600px] mx-auto py-8 px-4 sm:px-6">
      ...
      <p className="text-h4 text-text-secondary font-normal mb-8 max-w-3xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        <div className="lg:col-span-2 space-y-12">
          {/* Main Image */}
          <div className="rounded-3xl overflow-hidden shadow-warm-md h-100 md:h-125">
            <img
                    ...
                    {campaign.timeline.map((event, idx) => (
                      ...
                      >
                        ...
                        <div className="w-[calc(100%-4rem)] p-6 rounded-2xl border border-border bg-surface shadow-sm">
                          <time className="text-xs font-bold text-accent mb-2 block tracking-wider uppercase">
          ...
          <div className="bg-surface p-8 rounded-3xl shadow-warm-lg border border-border sticky top-24">
            ...
            <div className="flex justify-between items-center mb-6 bg-background rounded-xl p-4 border border-border">
              ...
              <div className="flex items-center gap-1.5">
                <span
                  className={`text-lg font-bold ${healthScore >= 70 ? "text-success" : healthScore >= 40 ? "text-warning" : "text-highlight"}`}
                >
                  {healthScore}
                </span>
            ...
            <div className="grid grid-cols-2 gap-4 mb-8 text-center py-6 border-y border-border">
              <div>
                <p className="text-h3 font-semibold">
                  {campaign.milestones?.filter((m) => m.reachedAt)?.length || 0}
                </p>
          ...
          <div className="bg-surface p-6 rounded-3xl border border-border flex items-center gap-4">
            <div className="w-16 h-16 rounded-full bg-border overflow-hidden">
              {campaign.creator?.avatar?.url ? (
                <img
                  src={campaign.creator.avatar.url}
            ...
            <div className="flex-1">
              ...
              <div className="mt-1">
                <TrustBadge
                  score={campaign.creator?.trustScore || 0}
