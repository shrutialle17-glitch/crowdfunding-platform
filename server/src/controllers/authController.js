...
exports.register = async (req, res, next) => {
  try {
    ...
    if (existingUser) {
      return res.status(409).json({ success: false, error: { message: 'Email already in use', code: 'DUPLICATE_EMAIL' } });
    }

    // Password validation: min 8 chars, 1 uppercase, 1 number
    const passwordRegex = /^(?=.*[A-Z])(?=.*\d).{8,}$/;
    if (!passwordRegex.test(password)) {
      return res.status(400).json({ success: false, error: { message: 'Password must be at least 8 characters long, contain 1 uppercase letter, and 1 number.', code: 'WEAK_PASSWORD' } });
    }
...
exports.login = async (req, res, next) => {
  try {
    ...
    if (!email || !password) {
      return res.status(400).json({ success: false, error: { message: 'Please provide email and password', code: 'BAD_REQUEST' } });
    }

    const user = await User.findOne({ email });
    if (!user) {
      return res.status(401).json({ success: false, error: { message: 'Invalid email or password', code: 'UNAUTHORIZED' } });
    }
...
exports.refresh = async (req, res, next) => {
  try {
    ...
    if (error.name === 'TokenExpiredError') {
       return res.status(403).json({ success: false, error: { message: 'Refresh token expired', code: 'FORBIDDEN' } });
    }
    next(error);
  }
};
